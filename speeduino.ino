/* Fevrier 2020 : ajout Varlauch et case 52 Teenst3.5 MicroEFI 
...
void loop()
{
    ...
    if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_15HZ)) //Every 32 loops
    {
      ...
      //********************************************************************
      //Oil pressure safety with pressure switch or analog sensor
      if (configPage6.OilSensorType < 1) // analog
        {
          readVARpinOilADC();          
          byte OilPressMiniAnalogvalue = 0;
          OilPressMiniAnalogvalue = table2D_getValue(&OilPressMiniAnalogTable, currentStatus.RPM / 100);
          if (configPage6.OilEnable > 0 && BIT_CHECK(currentStatus.engine, BIT_ENGINE_RUN) &&(currentStatus.VAROil) < (OilPressMiniAnalogvalue))
          {
            digitalWrite(pinWarnOil, HIGH);
            if (configPage6.OilEngineOFF > 0)
              {
              while (BIT_CHECK(currentStatus.engine, BIT_ENGINE_RUN))          
                {
                BIT_SET(currentStatus.spark, BIT_SPARK_BOOSTCUT);
                BIT_SET(currentStatus.status1, BIT_STATUS1_BOOSTCUT);           
                }              
                ...
                {
                digitalWrite(pinWarnOil, LOW); 
                BIT_CLEAR(currentStatus.spark, BIT_SPARK_BOOSTCUT);
                BIT_CLEAR(currentStatus.status1, BIT_STATUS1_BOOSTCUT);             
                }
        } 
      if (configPage6.OilSensorType > 0) // on/off
        {
          previousOilTrigger = OilTrigger;        
          if(configPage6.OilHiLo > 0) { OilTrigger = digitalRead(pinOil); }
          else { OilTrigger = !digitalRead(pinOil); }
          if (configPage6.OilEnable > 0 && OilTrigger > 0 && (currentStatus.RPM > (unsigned int)(configPage6.OilRPMMini)*100)) 
            {
            digitalWrite(pinWarnOil, HIGH); 
            if (configPage6.OilEngineOFF > 0)
              {
