...
void readVARpinOilADC()
{
  unsigned int tempReading;
  #if defined(ANALOG_ISR)
    tempReading = fastMap1023toX(AnChannel[pinOil-A5], 255); //Get the current raw oil pressure value
  #else
    tempReading = analogRead(pinOil);
    tempReading = fastMap1023toX(analogRead(pinOil), 255); //Get the current raw oil pressure value
  #endif
  currentStatus.VAROilADC = tempReading;
  currentStatus.VAROil = map(currentStatus.VAROilADC, 0, 255, ((configPage6.OilcalibrateMin)), ((configPage6.OilcalibrateMax)));
}
