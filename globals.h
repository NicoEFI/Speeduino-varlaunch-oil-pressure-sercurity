...
extern struct table2D OilPressMiniAnalogTable;
...
extern bool OilTrigger;
extern bool previousClutchTrigger;
extern bool previousOilTrigger;
...
#define pinIsOutput(pin)    ( ((pin) == pinFuelPump) || ((pin) == pinFan) || ((pin) == pinVVT_1) || ((pin) == pinVVT_2) || ((pin) == pinBoost) || ((pin) == pinIdle1) || ((pin) == pinIdle2) || ((pin) == pinTachOut) || ((pin) == pinWarnOil) )
...
struct statuses {
  ...
  int VARlaunchADC;
  int VAROilADC;
  byte OilcalibrateMin;
  byte OilcalibrateMax;
  int dwell;
...
struct config6 {
  ...
  byte OilWarnPin : 6;
  byte OilEngineOFF : 1;
  byte OilSensorType : 1;
  byte OilRpmAnalog[4];
  byte OilPressMiniAnalog[4]; 
  byte OilcalibrateMin;
  byte OilcalibrateMax;
...
static_assert(sizeof(struct config6) == 143, "configPage6 size is not 143");
