...
{   
    ...
    OilPressMiniAnalogTable.valueSize = SIZE_BYTE;
    OilPressMiniAnalogTable.axisSize = SIZE_BYTE; //Set this table to use byte axis bins
    OilPressMiniAnalogTable.xSize = 4;
    OilPressMiniAnalogTable.values = configPage6.OilPressMiniAnalog;
    OilPressMiniAnalogTable.axisX = configPage6.OilRpmAnalog;
...
void setPinMapping(byte boardID)
{
  switch (boardID)
  {
   ...
   case 31:
    ...
    case 52:
      ...
      pinOil = A5; // switch or analog ; A5 pour test mais sur PCB V3 A18 = 37 Pin Oil pressure
      pinWarnOil = 26; // Pin warning Leds low pressure Oil
  ...
  if ( (configPage6.OilPin != 0) && (configPage6.OilPin < BOARD_NR_GPIO_PINS) ) { pinOil = pinTranslate(configPage6.OilPin); }
  if ( (configPage6.OilWarnPin != 0) && (configPage6.OilWarnPin < BOARD_NR_GPIO_PINS) ) { pinWarnOil = pinTranslate(configPage6.OilWarnPin); } 
  ...
  if(configPage10.fuel2Mode == FUEL2_MODE_INPUT_SWITCH)
  {
   ...
   if(configPage6.OilEnable > 0 && configPage6.OilSensorType > 0) 
  {
    if (configPage6.OilPullRes == true) { pinMode(pinOil, INPUT_PULLUP); }
    else { pinMode(pinOil, INPUT); } //If Oil Pull Resistor is not set make input float.
  }
  
   if(configPage6.OilEnable > 0 && configPage6.OilSensorType < 1)
  {
    pinMode(pinOil, INPUT); //If Oil sensor analog type
  }
